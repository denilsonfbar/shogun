...
#pragma omp parallel shared(shortest_distances,neighbors,begin,callback,N) 
	{
		bool* f = new bool[N];
		bool* s = new bool[N];
		IndexType k;
...
#pragma omp parallel shared(shortest_distances,begin,landmarks,neighbors,callback,N_landmarks,n_neighbors)
	{
		bool* f = new bool[N];
		bool* s = new bool[N];
		IndexType k;
